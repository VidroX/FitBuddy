...
async def refresh(user: User = Depends(refresh_token_required)):
    return Tokens(
        access=JWTHelper.encode_user_token(
            user_id=str(user.id), is_refresh=False),
        refresh=None
    )
...
async def register(
    ...
    field_validator.add_regex(password, "password", r"^(?=.*[A-Z].*[A-Z])(?=.*[!@#$&*])(?=.*[0-9]).{6,}$",
                              "Password should be minimum 6 characters long, have 2 uppercase letters, 1 special character and 1 number")

    if gender not in Gender:
        field_validator.add(
            "gender", "Provided gender is not currently supported")

    existing_user = await UserModel.find_one(UserModel.email == email)

    if existing_user is not None:
        field_validator.add(
            "email", "User with provided E-Mail already registered.")
    ...
    if uploaded_images is None:
        raise HTTPException(status_code=400, detail={
                            "message": "Unable to process uploaded images. Please, contact support for assistance."})
    ...
    return TokenizedUserResponse(
        user=User(
            **new_user.dict(exclude={'password': True, "id": True}), id=str(new_user.id)),
        tokens=Tokens(
            access=JWTHelper.encode_user_token(
                user_id=str(new_user.id), is_refresh=False),
            refresh=JWTHelper.encode_user_token(
                user_id=str(new_user.id), is_refresh=True)
        )
    )
...
async def login(email: str = Form(default=""), password: str = Form(default="")):
    ...
    if not db_user:
        raise HTTPException(status_code=400, detail={
                            "message": "User not found with provided E-Mail and Password combination."})
    ...
    return TokenizedUserResponse(
        user=User(
            **db_user.dict(exclude={'password': True, "id": True}), id=str(db_user.id)),
        tokens=Tokens(
            access=JWTHelper.encode_user_token(
                user_id=str(db_user.id), is_refresh=False),
            refresh=JWTHelper.encode_user_token(
                user_id=str(db_user.id), is_refresh=True)
        )
    )
